...
def step_import():
    ...
    media_type = st.radio(
        "Media Type:",
        ["🎵 Audio (MP3, WAV, M4A)", "📸 Images (JPG, PNG)"],
        horizontal=True
    )
    
    type_map = {
        "🎵 Audio (MP3, WAV, M4A)": "audio",
        "📸 Images (JPG, PNG)": "image"
    }
    ...
    with col2:
        if st.button("Next: Auto-Generate →", type="primary", disabled=len(st.session_state.media_files) == 0):
...
def step_generate():
    ...
    st.header("✨ Step 2: Auto-Generate Content")
    ...
    if st.button("✨ Auto-Generate Suggestions", type="primary"):
        with st.spinner("Generating and processing content..."):
            ...
            st.success(f"✓ Generated and processed {len(clips)} clips!")
    
    if st.session_state.generated_clips:
        st.subheader("Generated Content")
        for i, clip in enumerate(st.session_state.generated_clips, 1):
            st.text(f"{i}. {clip['file'].name} → {clip['duration']}s clip")
    ...
    with col1:
        if st.button("← Back"):
            ...
            st.rerun()
    with col2:
        if st.button("Next: Edit →", type="primary", disabled=len(st.session_state.generated_clips) == 0):
...
def step_edit():
    ...
    st.header("✂️ Step 3: Manual Edit Studio")
    ...
    if st.session_state.generated_clips:
        ...
        if selected:
            ...
            if st.session_state.media_type == 'audio':
                st.subheader("🎵 Audio Timeline Controls")
                
                # Show current clip info
                st.info(f"📄 File: {clip['file'].name} | Current: {clip.get('start_time', 0)}s - {clip.get('start_time', 0) + clip.get('duration', 10)}s")
                ...
                st.caption(f"📍 Will capture: {start_time}s → {end_time}s ({duration} seconds)")
                ...
                st.info("💡 Photo editing tools (crop, brightness, contrast) coming soon!")
            
            if st.session_state.media_type == 'audio':
                if st.button("✓ Apply Changes & Reprocess", type="primary"):
                    # Update clip data
                    ...
                    with st.spinner("Reprocessing audio clip..."):
                        ...
                        try:
                            ...
                            st.success("✅ Changes applied! Audio reprocessed and hashtags will regenerate.")
                            
                        except Exception as e:
                            st.error(f"❌ Error reprocessing clip: {e}")
...
def step_preview():
    ...
    st.header("📱 Step 4: Platform Previews")
    ...
    if st.session_state.generated_clips:
        ...
        if clip_to_preview:
            ...
            if st.session_state.media_type == 'audio':
                ...
                st.info(f"🎵 Clip Duration: {clip.get('duration', 10)}s | Start: {clip.get('start_time', 0)}s")
            elif st.session_state.media_type == 'image':
                st.image(uploaded_file, use_container_width=True)
                st.info(f"📸 Image ready for posting")
    ...
    if not st.session_state.hashtags_generated:
        ...
        st.success("✅ Hashtags generated for all platforms!")
    ...
    for tab, platform in zip(tabs, st.session_state.selected_platforms):
        with tab:
            ...
            if hashtag_value:
                ...
                st.caption(f"🏷️ {hashtag_count} hashtags")
            
            col1, col2 = st.columns(2)
            with col1:
                if st.button("🔄 Regenerate Hashtags", key=f"regen_{platform}"):
                    hashtags = st.session_state.caption_gen.generate_hashtags(
                    ...
                    st.rerun()
            with col2:
                if st.button("✓ Approve", key=f"approve_{platform}", type="primary"):
                    st.success("Approved!")
...
def step_export():
    ...
    st.header("🚀 Step 5: Export Content")
    ...
    if download_media:
        st.subheader("💾 Download Your Media Files")
    ...
    st.subheader("📅 Generate Scheduler Import File")
    st.caption("CSV file includes captions, hashtags, and filenames - import this into your scheduler")
    
    if st.button("🚀 Generate Scheduler Export", type="primary"):
        with st.spinner("Creating scheduler files..."):
                ...
                for file_type, file_path in export_files.items():
                    if Path(file_path).exists():
                        with open(file_path, 'rb') as f:
                            st.download_button(
                                label=f"⬇️ Download {file_type.upper()}",
                                data=f.read(),
            ...
            st.info(f"💡 **Next Steps:**\n1. ⬇️ Download your audio clips/photos above (if not done yet)\n2. ⬇️ Download the {scheduler_type} CSV file\n3. 📤 Import CSV into {scheduler_type}\n4. 📤 Upload your audio clips/photos to {scheduler_type}\n5. 📅 Schedule your posts!")
...
def main():
    ...
    st.markdown("""
        <div style='text-align: center; color: #7f8c8d; padding: 20px;'>
            <p>Offline-first • No auto-posting • Scheduler-ready exports</p>
        </div>
    """, unsafe_allow_html=True)
